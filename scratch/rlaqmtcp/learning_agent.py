...
for e in range(total_episodes):
	...
	with open("log/rewards-"+str(e)+".csv","w") as rewardFile:
		rewardFile.write("time;delayMs;Util;dropProb;Dq;reward\n")
	...
	if (rewardsum > maxrew):
		maxrew = rewardsum
		print("the best episode : " , e)
		agent0.save_model('best_model.h5')
	time_history.append(time)
	rew_history.append(rewardsum)
	dropofprob.append(DRop)
	print(dropofprob)
...
print(obj)
print(dropofprob)
print (len(dropofprob))

plt.figure()
plt.bar(np.arange(len(obj)), dropofprob, align='center', alpha=0.5)
plt.xticks(np.arange(len(obj)), obj)
plt.ylabel('Number of drops')
plt.xlabel('Episode')
plt.title('Episodes Drops')
plt.savefig("Dropsbar.pdf")
